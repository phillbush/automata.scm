...
(define third-to-last-is-1
  (make-nfa
    ...
    (lambda (state) (eq? state 3))
    (lambda (symbol state)
      (cond ((and (eq? state 0) (eq? symbol #\0)) (set eq? 0))
            ((and (eq? state 0) (eq? symbol #\1)) (set eq? 0 1))
            ((and (eq? state 1) (eq? symbol #\0)) (set eq? 2))
            ((and (eq? state 1) (eq? symbol #\1)) (set eq? 2))
            ((and (eq? state 2) (eq? symbol #\0)) (set eq? 3))
            ((and (eq? state 2) (eq? symbol #\1)) (set eq? 3))
            (else (set eq?))))))
...
(define k-zeroes
  (make-nfa
    ...
    (lambda (symbol state)
      (cond ((and (eq? state 0) (null? symbol)) (set eq? 1 3))
            ((and (eq? state 1) (eq? symbol #\0)) (set eq? 2))
            ((and (eq? state 2) (eq? symbol #\0)) (set eq? 1))
            ((and (eq? state 3) (eq? symbol #\0)) (set eq? 4))
            ((and (eq? state 4) (eq? symbol #\0)) (set eq? 5))
            ((and (eq? state 5) (eq? symbol #\0)) (set eq? 3))
            (else (set eq?))))))
